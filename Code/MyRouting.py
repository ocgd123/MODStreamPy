...
def get_Modflow_Heads(sim,rvr_Network):
    
    head = sim.get_model().output.head().get_data()[0][0]
    for reach in rvr_Network.keys():
        gw_cell = rvr_Network[reach].reach_cell
        rvr_Network[reach].hgw = head[gw_cell]

def write_VTK(river_Network,ws):
    
    qgwfs = np.zeros(mesh.GetNumberOfCells())
    hstage = np.zeros(mesh.GetNumberOfCells())
    for reach in river_Network.reaches.keys():
        qgwfs[river_Network.reaches[reach].reach_cell] = river_Network.reaches[reach].qgwf
        hstage[river_Network.reaches[reach].reach_cell] = river_Network.reaches[reach].hriv + river_Network.reaches[reach].rbd_elev
        
    if  "Results" not in os.listdir(ws):
        file_path  = os.path.join(ws, "Results")
        os.mkdir(file_path)

    
    simulation_result = mesh.copy()
    simulation_result.cell_data["gw_head (m)"]=sim.get_model().output.head().get_data()[0][0]
    simulation_result.save(os.path.join(result_path,'sim_gw_head_myrout'+prjct_name+'.vtu'))
    

    simulation_resulta = mesh.copy()
    simulation_resulta.cell_data["q_leak_(m3/s)"]=qgwfs
    simulation_resulta.save(os.path.join(result_path,'sim_qleak_myrout'+prjct_name+'.vtu'))
    
    simulation_resulta = mesh.copy()
    simulation_resulta.cell_data["h_stage (m)"]=hstage
    simulation_resulta.save(os.path.join(result_path,'sim_stage_myrout.'+prjct_name+'.vtu'))  
